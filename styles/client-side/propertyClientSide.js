async function bookNow(checkin,checkout,id,pp,pName,pImage) {
 ...
 // code to hit booking api // http://localhost:3000/booking //post request //send obj in req.body
          ...
          const result = await response.json();   
           //once booking is created in pending state , initiate checkout flow by hitting payment checkout api
          if(result){
            window.location.href= `${result.redirectUrl}?bookingId=${result.booking}&totalPrice=${result.totalPrice}&pName=${pName}&pImage=${pImage}`;
          }      
}
